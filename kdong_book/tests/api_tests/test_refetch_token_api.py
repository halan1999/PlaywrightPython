...
def test_login_api(api_context):
    ...
    with allure.step(f"Send POST request to /api/refetch-token"):
        response = api_context.post("/api/refetch-token", data=payload_dict)
        allure.attach(response.text(), name="Response Body", attachment_type=allure.attachment_type.JSON)
        allure.attach(
            body=f"Response Status: {response.status}",
            name="Response Status",
    ...
    with allure.step("Verify status code and message"):
        ...
        print(response.json())
        assert res.message == "Refetch token successfully.", f"Lỗi message: {res.message}"
