...
class UserMenuComponent:
    ...
    def open_menu(self):
        dropdown = self.page.locator(USER_DROPDOWN)
        expect(dropdown).to_be_visible()
        dropdown.click()

    def go_to_profile(self):
        self.open_menu()
        profile = self.page.locator(PROFILE_ITEM)
    ...
    def change_avatar(self, filename):
        ...
        # Click avatar image
        avatar_icon = self.page.locator(AVATAR_ICON)
        expect(avatar_icon).to_be_visible()
        avatar_icon.click()

        # Click change profile picture (+ icon)
        change_icon = self.page.locator(CHANGE_ICON)
        expect(change_icon).to_be_visible()
        change_icon.click()

        # Build dynamid path
        root_dir = os.path.dirname(os.path.dirname(__file__))
        image_path = os.path.abspath(os.path.join(root_dir,"tests" ,"data", filename))
    ...
    def logout(self):
        self.open_menu()
        logout_btn = self.page.locator(LOGOUT_ITEM)
